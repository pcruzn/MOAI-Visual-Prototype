...
.directive("solrFacetGroup", function() {
  return {
    ...
    transclude: true,
    templateUrl:"angular-solr/view/solr_facet_group.html",
        ...
        function ( newVal, oldVal){
          if ( newVal !== oldVal ) {
            for (var k in facetGroupCtrl.getFacets()){
                if(solrCtrl.facet_fields.hasOwnProperty(k)){
                    facetGroupCtrl.setFacetResult(k, solrCtrl.facet_fields[k]);
                }else{
                    console.log("facet_fields doesn't have key:"+k)
                }
            }
          }
        }
...
.directive("solrSelected", function(){
  return {
    ...
    transclude: true,
    templateUrl: "angular-solr/view/solr_selected.html",
    require:"^solr",
...
.directive("solrSearch", function($location) {
  return {
    ...
    restrict: "E",
    templateUrl:"angular-solr/view/solr_search.html",
    require: "^solr",
    link: function( scope, element, attrs, ctrl){
...
.directive("solrFacet", function() {
  return {
    ...
    templateUrl:"angular-solr/view/solr_facet.html",
    link: function( scope, element, attrs, ctrl){
      ...
      scope.isEmpty = function() {
        try {
          ...
          return (Object.getOwnPropertyNames(scope.results).length === 0);
        }
        catch(err) {
          console.log("getOwnPropertyNames:'"+scope+"'");
          return false
        }
...
.directive("solrFacetResult", function($location) {
  return {
    ...
    require: "^solr",
    templateUrl:"angular-solr/view/solr_facet_result.html",
    link: function( scope, element, attrs, ctrl){
